"""
Unit tests for ClaimCalculator - These tests WILL FAIL due to the bug
"""
import pytest
...
class TestClaimCalculator:
    ...
    def test_grace_period_with_zero_days_should_fail(self):
        ...
        When grace_period_days = 0, the calculator should handle it gracefully
        and return a valid payout, but instead it crashes with ZeroDivisionError.
        
        Expected: Should calculate payout with maximum penalty (e.g., factor 3.0)
        Actual: Crashes with ZeroDivisionError because grace_period_factor = 0
        """
        policy = Policy(
            policy_id="P003",
            policy_type="home",
            status="grace_period",  # Status is grace_period
            deductible=1000.0,
            coverage_limit=50000.0,
            payout_rate=0.85,
            claim_count=2,
            total_claimed=5000.0,
            grace_period_days=0  # BUG: This is 0!
        )
        ...
        # Expected: Should return a valid payout (e.g., ~1020.0 with max penalty)
        # Actual: Will crash with ZeroDivisionError
        payout = self.calculator.calculate_payout_amount(policy, claim)
        assert payout > 0
