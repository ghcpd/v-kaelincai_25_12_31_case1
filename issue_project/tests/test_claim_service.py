...
class TestClaimServiceIntegration:
    ...
    def test_grace_period_claim_with_zero_days_integration_failure(self):
        """
        INTEGRATION TEST FAILURE: This test demonstrates the bug in full workflow
        
        When a policy is in grace_period with 0 days, the system should process
        the claim with maximum penalty, but instead crashes with ZeroDivisionError.
        
        Expected: Claim approved with valid payout amount
        Actual: Entire pipeline crashes, returning HTTP 500 to user
        """
        ...
        policy = Policy(
            ...
            grace_period_days=0  # BUG TRIGGER: 0 days in grace period
        )
        self.service.add_policy(policy)
