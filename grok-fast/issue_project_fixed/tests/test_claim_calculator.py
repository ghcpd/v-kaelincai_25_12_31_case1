...
class TestClaimCalculator:
    ...
    def test_grace_period_with_zero_days_maximum_penalty(self):
        """
        Test grace period with 0 days - applies maximum penalty
        
        When grace_period_days = 0, the calculator applies maximum penalty factor (3.0)
        to discourage claims when grace period has fully expired.
        
        Expected: Calculate payout with maximum penalty factor
        """
        policy = Policy(
            policy_id="P003",
            policy_type="home",
            ...
            grace_period_days=0  # 0 days remaining
        )
        ...
        # Expected: (5000 - 1000) * 0.85 * 0.90 / 3.0 = 4000 * 0.85 * 0.90 / 3.0 = 3400 * 0.90 / 3.0 = 3060 / 3.0 = 1020
        payout = self.calculator.calculate_payout_amount(policy, claim)
        assert payout == 1020.0
