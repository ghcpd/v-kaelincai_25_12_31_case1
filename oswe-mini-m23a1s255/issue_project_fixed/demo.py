"""
Demonstration script for the fixed insurance claim processing system.
All demos should run without raising an exception (including the previous
grace-period-with-zero-days case).
"""
...
def demo_grace_period_zero_days_fixed():
    """Previous bug: this used to raise ZeroDivisionError — now it succeeds."""
    print("=" * 60)
    ...
    payout = calculator.calculate_payout_amount(policy, claim)
    print(f"✓ Success! Policy {policy.policy_id} processed without error")
...
def demo_integration_test():
    ...
    # Add problematic policy (previously crashed)
    policy = Policy(
        policy_id="POL_999",
        policy_type="home",
        status="grace_period",
...
if __name__ == "__main__":
    ...
    print('All demos completed — no unhandled exceptions.')
