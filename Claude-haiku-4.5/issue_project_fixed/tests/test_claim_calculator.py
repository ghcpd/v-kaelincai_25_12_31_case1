"""
Unit tests for ClaimCalculator - FIXED VERSION (all tests should pass)
"""
import pytest
...
class TestClaimCalculator:
    ...
    def test_grace_period_with_zero_days_should_pass(self):
        """
        TEST NOW PASSES: This test previously demonstrated the bug
        
        When grace_period_days = 0, the calculator now handles it gracefully
        by applying the maximum penalty factor (3.0).
        
        Expected: Claim is approved with payout calculated using factor 3.0
        Actual: Returns valid payout amount with no errors
        """
        policy = Policy(
            policy_id="P003",
            policy_type="home",
        ...
        # Expected: (5000 - 1000) * 0.85 * 0.90 / 3.0 = 1020
        # Step by step:
        # 1. After deductible: 5000 - 1000 = 4000
        # 2. With payout rate: 4000 * 0.85 = 3400
        # 3. With penalty: 3400 * 0.90 = 3060
        # 4. With grace period: 3060 / 3.0 = 1020.0
        payout = self.calculator.calculate_payout_amount(policy, claim)
        assert payout == 1020.0
        assert payout > 0
