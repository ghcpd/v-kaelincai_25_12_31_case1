"""
Integration tests for ClaimService - FIXED VERSION (all tests should pass)
"""
import pytest
...
class TestClaimServiceIntegration:
    ...
    def test_grace_period_claim_with_zero_days_integration_passes(self):
        """
        INTEGRATION TEST NOW PASSES: This test previously demonstrated the bug
        
        When a policy is in grace_period with 0 days, the system now processes
        the claim successfully with the maximum grace period penalty.
        
        Expected: Claim approved with valid payout amount
        Actual: System processes claim successfully and returns approved result
        """
        ...
        policy = Policy(
            ...
            grace_period_days=0  # FIXED: Now handled properly
        )
        self.service.add_policy(policy)
        ...
        # Expected: Should return approved result with valid payout
        # Calculation: (8000 - 1500) * 0.9 * 0.95 / 3.0 = 5557.5 / 3.0 = 1852.5
        result = self.service.submit_claim(claim_data)
        assert result['status'] == 'approved'
        assert result['approved_amount'] == 1852.5  # (8000 - 1500) * 0.9 * 0.95 / 3.0
        assert 'claim_id' in result
        assert result['claim_id'] == 'C103'
    ...
    def test_grace_period_with_valid_days(self):
        ...
        # Expected: (2500 - 400) * 0.85 * 1.0 / 1.5 = 892.5
        assert result['status'] == 'approved'
