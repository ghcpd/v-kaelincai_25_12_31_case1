...
class ClaimCalculator:
    ...
    FIXED: Division by zero issue resolved
    """
    ...
    def calculate_payout_amount(self, policy: Policy, claim: Claim) -> float:
        ...
        # Step 4: Apply grace period adjustment
        if policy.is_in_grace_period():
            grace_period_factor = self._calculate_grace_period_factor(policy)
            # FIX: grace_period_factor is never 0, so division is safe
    ...
    def _calculate_grace_period_factor(self, policy: Policy) -> float:
        """
        Calculate grace period factor for penalty application
        
        FIX: Now handles the edge case when grace_period_days is 0
        
        Business logic:
        - Grace period >= 30 days: factor = 1.5 (50% payout reduction)
        - Grace period >= 15 days: factor = 2.0 (100% payout reduction)
        - Grace period > 0 and < 15 days: factor = 2.5 (150% payout reduction)
        - Grace period = 0 days: factor = 3.0 (200% payout reduction - maximum penalty)
        
        The factor is used as a divisor: payout_with_penalty / grace_period_factor
        Higher factor = greater penalty = lower payout
        
        Args:
            policy: The insurance policy
            
        Returns:
            float: Grace period penalty factor (always > 0)
        """
        if policy.grace_period_days >= 30:
            ...
            # FIX: Return 3.0 instead of 0 when grace_period_days is 0
            # This represents maximum penalty for policies with no grace period buffer
            return 3.0
